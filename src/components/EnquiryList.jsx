...
const EnquiryList = ({
  ...
  const filteredEnquiries = enquiries.filter((e) => {
    ...
    if (activeTab === "new" && hideIrrelevant) {
      if (e.status === "irrelevant") return false;
      ...
      <div className="space-y-5 animate-fade-in w-full">
        ...
        <div className="bg-white p-2 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex flex-wrap items-center gap-5 w-full">
            {/* Row 1: Search Bar */}
            <div className="relative w-full lg:flex-1 lg:max-w-[400px] shrink-0">
              <Search
                size={16}
              ...
              <input
                type="text"
                placeholder={`Search ${activeTab === "new" ? "Inbox" : activeTab === "hold" ? "On Hold" : "Dismissed"}...`}
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full pl-11 pr-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-sm font-bold focus:outline-none focus:ring-4 focus:ring-secondary/10 focus:border-secondary transition-all"
              />
            </div>
            ...
            {/* AI Controls Container: Centered on mobile, right-aligned on desktop */}
            {activeTab === "new" && (
              <div className="flex flex-wrap items-center justify-center lg:justify-end gap-5 w-full lg:w-auto">
                {/* AI Analysis Toggle Group */}
                <div className="flex items-center gap-2.5">
                  <span
                    className={`text-[9px] font-extrabold  tracking-widest whitespace-nowrap ${aiAnalysisEnabled ? "text-primary" : "text-slate-400"}`}
                  >
                    AI Analysis
                  </span>
                  <button
                    onClick={() => {
                      ...
                      if (!nextValue) {
                        // Turning OFF AI Analysis: also turn off spam filter, restore, AND UNLABEL
                        setHideIrrelevant(false);
                        enquiries.forEach((enq) => {
                          // Unlabel all enquiries (clear AI results)
                          onUpdate({
                            ...enq,
                            AI_TRANSCRIPT: undefined,
                            aiAnalysis: undefined,
                            status:
                              enq.status === "dismissed" ||
                                enq.status === "irrelevant"
                                ? "new"
                                : enq.status,
                          });

                          // Also explicitly restore if they were in dismissed
                          if (
                            enq.status === "dismissed" ||
                            enq.status === "irrelevant"
                          ) {
                    ...
                    className={`relative inline-flex h-5 w-10 flex-shrink-0 cursor-pointer rounded-full transition-colors duration-200 ease-in-out focus:outline-none ${aiAnalysisEnabled ? "bg-secondary" : "bg-slate-200"}`}
                  >
                    <span
                      className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-md transition duration-200 ease-in-out ${aiAnalysisEnabled ? "translate-x-5" : "translate-x-1"} mt-0.5`}
                    />
                  </button>
                </div>

                {/* Filter Spam Toggle Group */}
                <div className="flex items-center gap-2.5">
                  <span
                    className={`text-[9px] font-extrabold  tracking-widest whitespace-nowrap ${aiAnalysisEnabled && hideIrrelevant ? "text-primary" : "text-slate-400"}`}
                  >
                    Filter Spam
                  </span>
                  <button
                    onClick={() => {
                      ...
                      const nextValue = !hideIrrelevant;
                      setHideIrrelevant(nextValue);
                      if (nextValue) {
                        // Automatically dismiss all currently irrelevant enquiries in the inbox
                        enquiries.forEach((enq) => {
                          if (
                            (enq.status === "new" ||
                              enq.status === "read" ||
                              enq.status === "irrelevant") &&
                            (enq.status === "irrelevant" ||
                              (enq.aiAnalysis && !enq.aiAnalysis.isRelevant))
                          ) {
                            onDismiss(enq.id);
                          }
                        });
                      } else {
                        // Automatically RESTORE all irrelevant enquiries back to the inbox
                        enquiries.forEach((enq) => {
                          if (
                            (enq.status === "dismissed" ||
                              enq.status === "irrelevant") &&
                    ...
                    className={`relative inline-flex h-5 w-10 flex-shrink-0 cursor-pointer rounded-full transition-colors duration-200 ease-in-out focus:outline-none ${!aiAnalysisEnabled ? "bg-slate-100 cursor-not-allowed" : hideIrrelevant ? "bg-primary" : "bg-slate-200"}`}
                  >
                    <span
                      className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-md transition duration-200 ease-in-out ${hideIrrelevant ? "translate-x-5" : "translate-x-1"} mt-0.5`}
                    />
                  </button>
                </div>

                {/* AI Model Settings & Re-analyze (Flattened for equal spacing) */}
                {aiAnalysisEnabled && (
                  <>
                    {/* Vertical Divider */}
                    ...
                    {aiModels.length > 0 && (
                      <div className="relative w-full lg:w-52">
                        <button
                          ...
                          className="w-full flex items-center justify-between gap-3 px-5 py-2.5 bg-white border-2 border-slate-900 rounded-full text-[10px] font-black  tracking-widest text-[#18254D] hover:bg-slate-50 transition-all shadow-md active:scale-95 group"
                        >
                        ...
                        {isModelDropdownOpen && (
                          <>
                            ...
                            />
                            <div className="absolute top-full right-0 mt-2 w-full bg-white border-2 border-slate-900 rounded-2xl shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 leading-none">
                              <div className="max-h-60 overflow-y-auto py-1">
                                {aiModels.map((model) => (
                                  <button
                                    ...
                                    className={`w-full text-left px-5 py-3 hover:bg-slate-50 transition-colors flex flex-col gap-1 ${selectedAiModel === model.modelId
                                      ? "bg-slate-50"
                                      : ""
                                      }`}
                                  >
                                    <div className="flex items-center justify-between">
                                      ...
                                      {selectedAiModel === model.modelId && (
                                        <div className="w-1.5 h-1.5 rounded-full bg-secondary" />
                                      )}
                                    </div>
                                    <span className="text-[8px] font-bold text-slate-400  tracking-tighter truncate">
                                      {model.description}
                                    </span>
                                  </button>
                                ))}
                    ...
                    <button
                      ...
                      className="w-full lg:w-auto flex items-center justify-center gap-2 px-6 py-2.5 bg-slate-900 text-white rounded-full text-[10px] font-black  tracking-widest hover:bg-slate-800 transition-all shadow-md active:scale-95 group"
                    >
                      <RefreshCcw
                        size={14}
                        strokeWidth={3}
            ...
            {["new", "hold", "dismissed"].map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`px-5 h-full rounded-xl text-[10px] font-bold  tracking-wider transition-all flex items-center justify-center min-w-[100px] border border-transparent whitespace-nowrap ${activeTab === tab
        ...
        <div className="flex flex-col gap-3 w-full">
          {filteredEnquiries.length === 0 ? (
            ...
            filteredEnquiries.map((enquiry, idx) => (
              <div
                key={`${enquiry.id}-${idx}`}
                className={`group bg-white rounded-2xl border transition-all hover:shadow-lg overflow-hidden flex flex-col w-full ${enquiry.aiAnalysis && !enquiry.aiAnalysis.isRelevant ? "border-error/10 bg-error/[0.01]" : "border-slate-200 hover:border-secondary/30"}`}
              >
                {/* Card Header: Name + Date + Badges */}
                <div className="px-4 md:px-5 pt-4 pb-3 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div
                      className={`w-9 h-9 rounded-2xl flex items-center justify-center text-white font-bold text-xs shadow-sm shrink-0 ${enquiry.status === "hold" ? "bg-warning" : enquiry.status === "dismissed" ? "bg-slate-400" : "bg-primary"}`}
                    >
                      {enquiry.name.charAt(0)}
                    </div>
                    <div className="min-w-0">
                      <h3 className="font-bold text-primary text-sm tracking-tight truncate leading-tight">
                        {enquiry.name}
                      </h3>
                      <div className="flex items-center gap-1.5 text-[9px] text-textMuted font-bold  tracking-widest mt-0.5">
                        <Clock size={10} className="text-secondary shrink-0" />
                  ...
                  <div className="flex flex-wrap gap-1.5 justify-end shrink-0">
                    {enquiry.holdReason && (
                      <div className="px-2.5 py-1 bg-warning/10 text-warning border border-warning/20 rounded-lg text-[7px] font-bold  tracking-widest flex items-center gap-1 shadow-sm">
                        <PauseCircle size={10} /> Reason Logged
                      </div>
                    )}
                    {enquiry.aiAnalysis &&
                      (() => {
                        const score = enquiry.aiAnalysis.leadScore;
                        const isRelevant = enquiry.aiAnalysis.isRelevant;
                        const category = enquiry.aiAnalysis.category;

                        const displayLabel = isRelevant
                          ? "Relevant"
                          : `Irrelevant${category && category !== "Irrelevant" ? ` : ${category}` : ""}`;
                        ...
                        return (
                          <div
                            className={`px-2.5 py-1 rounded-lg text-[10px] font-bold  tracking-widest border flex items-center gap-1 shadow-sm ${style}`}
                          >
                            <Sparkles size={12} />
                            {displayLabel}
                            {score !== undefined && (
                ...
                {/* Card Body: Contact Info + Message */}
                <div className="px-4 md:px-5 pb-3 flex flex-col lg:flex-row gap-3">
                  {/* Contact info - compact inline */}
                  <div className="lg:w-[200px] shrink-0 flex flex-col gap-1.5">
                    <div className="flex items-center gap-2 text-[11px] text-textMuted font-medium py-1.5 px-2.5 bg-slate-50 rounded-lg border border-slate-100 overflow-hidden">
                  ...
                  {/* Briefing message */}
                  <div className="flex-1 min-w-0">
                    <div className="bg-slate-50/70 p-4 rounded-2xl border border-slate-100 group-hover:bg-white group-hover:border-slate-200 transition-all">
                      <p className="text-xs md:text-[13px] text-primary font-medium italic leading-relaxed">
                        "{enquiry.message}"
                      </p>
                      {enquiry.holdReason && (
                        <div className="mt-3 pt-3 border-t border-slate-200/60">
                          ...
                          <p className="text-[11px] text-warning font-bold bg-warning/5 p-2.5 rounded-lg border border-warning/10">
                            {enquiry.holdReason}
                          </p>
                        </div>
                      )}
                ...
                {/* Action Bar */}
                <div className="px-4 md:px-5 py-3 bg-slate-50/50 border-t border-slate-100 flex flex-row flex-wrap items-center gap-3 group-hover:bg-white transition-colors">
                  {activeTab === "new" && (
                    <React.Fragment>
                      <button
                        onClick={() => openLeadModal(enquiry)}
                        className="flex-1 sm:flex-initial bg-[#18254D] text-white px-4 py-2.5 rounded-2xl text-[10px] font-bold  tracking-widest flex items-center justify-center gap-1.5 shadow-sm active:scale-95 hover:bg-slate-800 transition-all whitespace-nowrap"
                      >
                        <CheckCircle size={13} strokeWidth={2.5} /> Add to Lead
                      </button>
                      <button
                        onClick={() => openHoldModal(enquiry)}
                        className="flex-1 sm:flex-initial px-4 py-2.5 bg-white border border-slate-200 text-slate-500 rounded-2xl hover:text-warning hover:border-warning transition-all text-[10px] font-bold  tracking-widest flex items-center justify-center gap-1.5 whitespace-nowrap"
                      >
                        <PauseCircle size={13} /> Hold
                      </button>
                      <button
                        onClick={() => onDismiss(enquiry.id)}
                        className="flex-1 sm:flex-initial px-4 py-2.5 bg-white border border-slate-200 text-slate-500 rounded-2xl hover:text-error hover:border-error transition-all text-[10px] font-bold  tracking-widest flex items-center justify-center gap-1.5 whitespace-nowrap"
                      >
                        <X size={13} /> Dismiss
                      </button>
                    </React.Fragment>
                  ...
                  {activeTab === "hold" && (
                    <React.Fragment>
                      <button
                        ...
                        className="flex-1 sm:flex-initial bg-primary text-white px-4 py-2.5 rounded-2xl text-[10px] font-bold  tracking-widest flex items-center justify-center gap-1.5 shadow-sm active:scale-95 transition-all whitespace-nowrap"
                      >
                        <RefreshCcw size={13} strokeWidth={3} /> Restore Enquiry
                      </button>
                      <button
                        onClick={() => onDismiss(enquiry.id)}
                        className="flex-1 sm:flex-initial px-4 py-2.5 bg-white border border-slate-200 text-slate-500 rounded-2xl hover:text-error hover:border-error transition-all text-[10px] font-bold  tracking-widest flex items-center justify-center gap-1.5 whitespace-nowrap"
                  ...
                  {activeTab === "dismissed" && (
                    <React.Fragment>
                      <button
                        ...
                        className="flex-1 sm:flex-initial bg-primary text-white px-4 py-2.5 rounded-2xl text-[10px] font-bold  tracking-widest flex items-center justify-center gap-1.5 shadow-sm active:scale-95 transition-all whitespace-nowrap"
                      >
                        <RefreshCcw size={13} strokeWidth={2.5} /> Restore
                        Enquiry
                      </button>
                      <button
                        onClick={() => onDelete(enquiry.id)}
                        className="p-2.5 bg-white border border-slate-200 text-slate-300 rounded-2xl hover:text-error hover:border-error hover:bg-error/5 transition-all flex items-center justify-center"
                ...
                <div className="grid grid-cols-2 gap-2">
                  {["Tech", "Media"].map((cat) => (
                    <button
                      ...
                      className={`flex items-center justify-center py-2.5 rounded-2xl border-2 transition-all font-bold  text-[10px] tracking-widest ${promoteFormData.leadCategory === cat
                ...
                <div className="grid grid-cols-3 gap-2">
                  {["Hot", "Warm", "Cold"].map((type) => (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center gap-1 py-2 rounded-xl border-2 transition-all ${promoteFormData.leadType === type
                        ? type === "Hot"
